...
function setMysqlConfig() {
  return new Promise(async (resolve, reject) => {
    getSecretValue(CONFIG.aws_mysql_conf_key).then((configs) => {
      CONFIG.db_dialect = configs.DB_DIALECT;
      CONFIG.db_host = configs.DB_HOST;
      CONFIG.db_port = configs.DB_PORT;
      CONFIG.db_name = configs.DB_NAME;
      CONFIG.db_user = configs.DB_USER;
      CONFIG.db_password = configs.DB_PASSWORD;
      resolve('SUCCESS');
    }).catch((error) => {
...
function setMailServerConfig() {
  return new Promise(async (resolve, reject) => {
    getSecretValue(CONFIG.aws_mailserver_conf_key).then((configs) => {
      ...
      CONFIG.hostUrl = configs.HOSTURL;
      resolve('SUCCESS');
    }).catch((error) => {
...
function setJWTConfig() {
  return new Promise(async (resolve, reject) => {
    getSecretValue(CONFIG.aws_jwt_conf_key).then((configs) => {
      CONFIG.jwt_encryption = configs.JWT_ENCRYPTION;
      CONFIG.jwt_expiration = configs.JWT_EXPIRATION;
      CONFIG.refresh_token_expiration = configs.REFRESH_TOKEN_EXPIRATION;
      resolve('SUCCESS');
    }).catch((error) => {
